...
const buildAlert = (device, dataId, parameter, readingValue, min, max) => {
    ...
    return {
        ...
        severity: ['ph_value', 'water_temperature_c', 'air_temperature_c'].includes(parameter) ? 'high' : 'medium'
    };
};
...
const checkThresholds = async (device, config, sensorPayload) => {
    ...
    const parameterMap = [
        ...
        { key: 'light_intensity', min: config.light_intensity_min, max: config.light_intensity_max },
        { key: 'air_temperature_c', min: config.air_temp_min, max: config.air_temp_max },
        { key: 'water_temperature_c', min: config.water_temp_min, max: config.water_temp_max }
    ];
    ...
    for (const { key, min, max } of parameterMap) {
        ...
        const alertData = buildAlert(device, sensorPayload.data_id, key, value, min, max);
...
router.post('/', async (req, res) => {
    try {
        ...
        const waterTemp = parseValue(req.body.water_temperature_c ?? req.body.temperature_water_c);
        const airTemp = parseValue(req.body.air_temperature_c ?? req.body.temperature_air_c);
        ...
        const sensorPayload = {
            ...
            ph_value: phValue,
            ec_value: ecValue,
            tds_value: tdsValueInput ?? (ecValue ? Number((ecValue * 0.64).toFixed(2)) : undefined),
            water_temperature_c: waterTemp,
            air_temperature_c: airTemp,
            light_intensity: light
        };

        const requiredFields = ['ph_value', 'ec_value', 'tds_value', 'water_temperature_c', 'air_temperature_c', 'light_intensity'];
        const missingField = requiredFields.find((field) => sensorPayload[field] === undefined || Number.isNaN(sensorPayload[field]));

        if (missingField) {
            return res.status(400).json({ success: false, message: 'All sensor parameters are required' });
        }
...
router.get('/:deviceId/history', async (req, res) => {
    try {
        ...
        res.json(data.map(doc => attachVirtualMetrics(doc.toObject({ virtuals: true }))));
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
});
