      ...
      <div v-if="showStatistics" class="chart-card__stats">
        <div class="stat-item">
          ...
          <span class="stat-value"
            >{{ statistics.avg.toFixed(decimals) }} {{ unit }}</span
          >
        </div>
        <div class="stat-item">
          <span class="stat-label">Min</span>
          <span class="stat-value"
            >{{ statistics.min.toFixed(decimals) }} {{ unit }}</span
          >
        </div>
        <div class="stat-item">
          <span class="stat-label">Max</span>
          <span class="stat-value"
            >{{ statistics.max.toFixed(decimals) }} {{ unit }}</span
          >
        </div>
        <div class="stat-item">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // Dynamic optimal range color based on chart color
    const optimalRangeColor = computed(() => {
    ...
    const currentStatus = computed(() => {
      ...
      if (
        latestValue < props.optimalRange.min ||
        latestValue > props.optimalRange.max
      ) {
        return "critical";
      }
          ...
          datasets: [
            {
              ...
              backgroundColor:
                props.chartType === "area" ? `${props.color}33` : props.color,
        ...
        options: {
          ...
          scales: {
            x: {
              ...
              ticks: {
                maxTicksLimit: 6, // Reduced from 8 for longer date labels
                font: { size: 11 },
                maxRotation: 45,
                minRotation: 45,
              },
